...
const typeNames = ({ type }) => type('Bienvenu à  ', 'Motorizi ');
const typeTitles = time => ({ type }) => type(time, " site d'annonces ");
...
export default function HeaderText() {
  ...
  const isFirstImpression =
    !cookies[TYPEKEV_SITE_PREV_WELCOMED] && ['/', '/voitures/'].includes(window.location.pathname);
